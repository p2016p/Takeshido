...
public class takeshido_util_misc extends BaseModPlugin {
    private static org.apache.log4j.Logger log = Global.getLogger(takeshido_util_misc.class);
    ...
    public static void createAIPersona(PersonAPI commander, ShipAPI ship)
    {
        PersonAPI AI;
        String AIName;
        if(Global.getSector().getPersistentData().get("takeshido_AIPersona_"+ship.getCaptain().getId()) instanceof PersonAPI) {
            AI = (PersonAPI)Global.getSector().getPersistentData().get("takeshido_AIPersona_"+ship.getCaptain().getId());
            AIName = (String)Global.getSector().getPersistentData().get("takeshido_AIPersona_"+"name_"+ship.getCaptain().getId());
        }
        else{
            ...
            Global.getSector().getPersistentData().put("takeshido_AIPersona_"+ship.getCaptain().getId(),AI);
            Global.getSector().getPersistentData().put("takeshido_AIPersona_"+"name_"+ship.getCaptain().getId(),AIName);
        }
    }
    ...
    public static void createAnanke(PersonAPI commander, ShipAPI ship)
    {
        PersonAPI AI;
        String AIName;
        if(Global.getSector().getPersistentData().get("takeshido_Ananke_"+ship.getCaptain().getId()) instanceof PersonAPI) {
            AI = (PersonAPI)Global.getSector().getPersistentData().get("takeshido_Ananke_"+ship.getCaptain().getId());
            AIName = (String)Global.getSector().getPersistentData().get("takeshido_Ananke_"+"name_"+ship.getCaptain().getId());
        }
        else{
            AI = Misc.getAICoreOfficerPlugin("omega_core").createPerson("omega_core", "player", Misc.random);
            AIName = "Ananke";
            Global.getSector().getPersistentData().put("takeshido_Ananke_"+ship.getCaptain().getId(),AI);
            Global.getSector().getPersistentData().put("takeshido_Ananke_"+"name_"+ship.getCaptain().getId(),AIName);
        }
    }
